...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < num_gpus; ++i) {
    ...
    cudaMalloc(&dC_gpu[i], sub_m * n * sizeof(T));  // 分配C
  ...
  for (int i = 0; i < num_gpus; ++i) {
    ...
    cudaMemcpyAsync(dC_gpu[i], C + i * sub_m * n, sub_m * n * sizeof(T), cudaMemcpyHostToDevice, streams[i]);
  }
  ...
  for (int i = 0; i < num_gpus; ++i) {
    NCCLCHECK(ncclReduce((const void*)dC_gpu[i], (void*)(C + i * sub_m * n), sub_m * n, ncclFloat, ncclSum, 0, comms[i], streams[i]));
  }
